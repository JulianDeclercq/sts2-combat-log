...
All logic lives in `.claude/scripts/test-multiplayer.ps1`. It handles: build, steam_appid, `settings.save` patch (fullscreen=false), resolution detect, launch, and MoveWindow-based tiling with a 15s retry loop to outlast Godot's delayed window-position restore.
...
```bash
powershell -NoProfile -File .claude/scripts/test-multiplayer.ps1 -Players ${ARGUMENTS:-2} >/dev/null 2>&1 &
```
...
- **Script:** `.claude/scripts/test-multiplayer.ps1`
