...
public partial class CardDiscardRow : HBoxContainer
{
    ...
    private readonly CardDiscardEvent _entry;
    private readonly Action<CardModel> _openInspect;

    public CardDiscardRow(CardDiscardEvent entry, Action<CardModel> openInspect)
    {
        _entry = entry;
    ...
    public override void _Ready()
    {
        ...
        if (_entry.DiscardedCard is not null)
        {
            var tinyCard = TinyCardFactory.Build(_entry.DiscardedCard, CardIconSize);
            if (tinyCard is not null) AddChild(tinyCard);
        }

        var nameLabel = new Label();
        var shortCardName = NameTruncator.Short(_entry.DiscardedCardName);
        var nameText = string.IsNullOrEmpty(_entry.OwnerName)
            ? $"{shortCardName} discarded"
            : $"{shortCardName} discarded [{_entry.OwnerName}]";
        nameLabel.Text = nameText;
        var baseColor = _entry.DiscardedCard is null ? NoCardColor : LabelColor;
        nameLabel.AddThemeColorOverride("font_color", baseColor);
        AddChild(nameLabel);

        var card = _entry.DiscardedCard;
